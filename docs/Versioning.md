...
| Where | Value | Set by |
|---|---|---|
| `AssemblyVersion`, `FileVersion` | `0.1.0.0` | .NET SDK |
| `InformationalVersion` | `0.1.0+<commit>` | .NET SDK (Source Link appends the commit) |
| Dalamud manifest `AssemblyVersion` | `0.1.0.0` | DalamudPackager, read from the built assembly |

Don't write a version anywhere else. The tests fail if a project sets its own version.

## Seeing which build is loaded

- **`/aetherframe version`** or **`/af version`** prints the running build in chat, for example `AetherFrame 0.1.0 (build 1bf26e1)`.
